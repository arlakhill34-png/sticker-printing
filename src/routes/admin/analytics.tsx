...
import { ResponsiveContainer, Line, Bar, CartesianGrid, XAxis, YAxis, Tooltip as ReTooltip, Pie } from "recharts";
import {
    ...
    <AdminInner title="Analytics" subtitle="Subscription trends and user growth insights">
      <div className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-3">
          <Summary label="Total Active"   value={data.totalActive}    icon={BarChart3}  color="bg-blue-500/10 text-blue-500" hint="blue-500" />
          <Summary label="Expiring Soon"  value={data.expiringSoon}   icon={TrendingUp} color="bg-amber-500/10 text-amber-500" hint="amber-500" />
          <Summary label="Active Subs"    value={data.activeSubscriptions?.[data.activeSubscriptions.length - 1]?.count || 0}
            icon={LineChart} color="bg-emerald-500/10 text-emerald-500" hint="emerald-500" />
        </div>

        <TrendLine data={data.userGrowth} color="#1d4ed8" title="Registrations per Day" dataKey="userGrowth" />
        <TrendBar  data={data.activeSubscriptions} color="#059669" title="Active Subscriptions Trend" dataKey="activeSubs"  />
...
function Badge({ children, variant, className }: { children: React.ReactNode; variant?: "default" | "outline"; className?: string }) {
  const styles = variant === "outline" ? "bg-transparent border border-border text-foreground" : "bg-primary text-primary-foreground";
...
function TrendLine({ data, color, title, dataKey }: { data: { date: string; count: number }[]; color: string; title: string; dataKey: string }) {
  return (
    <Card className="rounded-xl border bg-card shadow-sm overflow-hidden">
      <CardHeader className="py-4"><CardTitle className="text-base font-semibold">{title}</CardTitle></CardHeader>
        ...
        <ChartContainer config={ccfg} className="h-72">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <Line data={data}>
              ...
              <XAxis dataKey="date" tick={{ fontSize: 11, fill: "var(--color-muted-foreground)" }} tickFormatter={(v: string) => v.slice(5)} />
              ...
              <Line type="monotone" dataKey="count" stroke={`var(--color-${dataKey})`} strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
            </Line>
          </ResponsiveContainer>
...
function TrendBar({ data, color, title, dataKey }: { data: { date: string; count: number }[]; color: string; title: string; dataKey: string }) {
  return (
    <Card className="rounded-xl border bg-card shadow-sm overflow-hidden">
      <CardHeader className="py-4"><CardTitle className="text-base font-semibold">{title}</CardTitle></CardHeader>
      <CardContent className="pt-0">
...
function BrePie({ data }: { data: { name: string; value: number; fill: string }[] }) {
  return (
    <Card className="rounded-xl border bg-card shadow-sm overflow-hidden">
      <CardHeader className="py-4"><CardTitle className="text-base font-semibold">Subscription Breakdown</CardTitle></CardHeader>
      <CardContent className="pt-0">
        <ChartContainer config={ccfg} className="h-64">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <Pie data={data}>
              <ReTooltip />
              <Pie dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90}
    ...
    <SidebarProvider defaultOpen={true}>
      ...
      <Sidebar className="hidden md:flex bg-sidebar border-sidebar-border">
        ...
        <SidebarFooter className="px-2 py-3">
          ...
          <button onClick={() => { useAuth().logout(); navigate({ to: "/login" }); }} className="flex items-center gap-2 w-full px-3 py-2.5 rounded-lg text-sm text-destructive hover:bg-destructive/10 transition-colors">
...
export { Separator, Avatar, AvatarFallback };
