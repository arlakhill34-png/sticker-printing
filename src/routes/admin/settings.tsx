...
function SettingsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error("Error", { description: "Failed to change password. Please verify your current password." });
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="space-y-6 max-w-2xl">
          ...
          <CardContent className="pt-0 space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="mt-1">
                  <Badge variant={user?.role === "ADMIN" ? "default" : "outline"} className="text-[11px]">{user?.role}</Badge>
                </div>
              </div>
            </div>
        ...
        <Card className="rounded-xl border bg-card shadow-sm">
          ...
          <CardContent className="pt-0 space-y-3">
            <div>
              ...
              <Input type="password" value={current} onChange={(e) => setCurrent(e.target.value)} placeholder="Enter current password" className="mt-1.5 h-9" />
            </div>
            <div>
              <Label className="text-xs font-medium text-muted-foreground">New Password</Label>
              <Input type="password" value={newPass} onChange={(e) => setNewPass(e.target.value)} placeholder="Enter new password" className="mt-1.5 h-9" />
            </div>
            <div>
              <Label className="text-xs font-medium text-muted-foreground">Confirm New Password</Label>
              <Input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder="Re-enter new password" className="mt-1.5 h-9" />
            </div>
            <Button onClick={handleSave} disabled={saving} className="mt-1">{saving ? "Saving…" : "Update Password"}</Button>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-xl border border-destructive/30 bg-destructive/5 shadow-sm">
          ...
          <CardContent className="pt-0">
            ...
            <Button variant="destructive" size="sm" onClick={() => { logout(); nave("login"); }}>
              Sign Out Everywhere
            </Button>
          </CardContent>
        </Card>
...
function AdminInner({ children, title, subtitle }: { children: React.ReactNode; title: string; subtitle?: string }) {
  const location = useLocation();
  const { user, logout } = useAuth();
  const navigate = useNavigate();
  const { isMobile, openMobile, setOpenMobile } = useSidebar();

  function nave(path: string) { window.location.href = path; }
